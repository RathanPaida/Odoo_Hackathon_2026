...
export function DepartmentForm({
  ...
  const onSubmit = async (data: FormValues) => {
    setError(null);
    try {
      const url = initialData ? `/api/admin/departments/${initialData.id}` : `/api/admin/departments`;
  ...
  return (
    <form onSubmit={handleSubmit(onSubmit)} className="space-y-4">
      ...
      <div>
        ...
        <input
          {...register("code")}
          className={clsx("mt-1 block w-full rounded-md border-slate-300 shadow-sm focus:border-brand-500 focus:ring-brand-500", errors.code && "border-red-500")}
